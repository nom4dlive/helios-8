...
import ExoExplorer from "./components/ExoExplorer";
...
export default function App() {
  ...
  /* atalhos: V = visitar superfície · X = mundos habitáveis · Esc = sair */
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (visitId) setVisitId(null);
        else if (exoOpen) setExoOpen(false);
        return;
      }
      if ((e.key === "v" || e.key === "V") && !visitId && !exoOpen && selectedId) {
        if (getSurfaceView(selectedId)) setVisitId(selectedId);
      }
      if ((e.key === "x" || e.key === "X") && !visitId && !exoOpen) {
        setExoOpen(true);
      }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [visitId, selectedId, exoOpen]);

  const handleVisit = (id: string) => {
    if (getSurfaceView(id)) setVisitId(id);
  };
  ...
  const visitBody = useMemo(() => {
    ...
    if (!view || visitId === "sun") return null;
    ...
    <div className="relative h-full w-full select-none overflow-hidden bg-void font-body text-ink">
      ...
      {!visitBody && !exoOpen && (
        <div className="pointer-events-none absolute bottom-4 left-4 z-20 hidden flex-col gap-0.5 font-mono text-[9px] tracking-[0.18em] text-dim md:flex">
          ...
          <span>CLIQUE NUM CORPO — DADOS · V — VISITAR SUPERFÍCIE · X — MUNDOS HWC</span>
      ...
      {/* catálogo de mundos habitáveis */}
      {exoOpen && <ExoExplorer onClose={() => setExoOpen(false)} />}
    </div>
  );
}
